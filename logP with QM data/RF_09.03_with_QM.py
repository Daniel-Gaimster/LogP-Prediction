...
stored_import = pd.DataFrame(pd.read_csv('features_template_QM.csv'))
stored_metrics = pd.DataFrame()

# Running model for n times
for n in range(runtimes):
    # Read in data as pandas dataframe
    features = pd.read_csv('mordredRF_with_QM.csv')
    ...
    train_features, test_features, train_labels, test_labels = train_test_split(features, labels, test_size = 0.25, random_state=seed*n)
    ...
    # Store values to csv
    stored_preds.to_csv("stored_predictions_w_QM.csv")
    stored_opt.to_csv("stored_opt_w_QM.csv")
    stored_import.to_csv("stored_import_w_QM.csv")
    stored_metrics.to_csv("stored_metrics_w_QM.csv")
