...
max_features = ['sqrt', 'log2', 0.333]  # (int(np.size(test_features, 1)/3) was removed due to problems with RFE, number features wasnt updating after RFE.
...
stored_preds = pd.read_csv('predictions_template_solv.csv')
...
for n in range(runtimes):
    ...
    features = pd.read_csv('mordredsolv.csv')

    # Label = the value we want to predict; solv
    labels = np.array(features['solv'])

    # Features = what the rf will use
    # Remove solv, axis = 1 refers to columns, unsure what it does
    features = features.drop('solv', axis=1)
    ...
    feature_list = np.delete(feature_list, np.where(feature_list == "solv"))
    ...
    # Store values to csv
    stored_preds.to_csv("stored_predictions_solv.csv")
    stored_opt.to_csv("stored_opt_solv.csv")
    stored_import.to_csv("stored_import_solv.csv")
    stored_metrics.to_csv("stored_metrics_solv.csv")
