...
max_features = ['sqrt', 'log2',
                0.333]  # (int(np.size(test_features, 1)/3) was removed due to problems with RFE, number features wasnt updating after RFE.
...
stored_preds = pd.read_csv('predictions_template_hyd.csv')
...
for n in range(runtimes):
    ...
    features = pd.read_csv('mordredhyd.csv')

    # Label = the value we want to predict; hyd
    labels = np.array(features['hyd'])

    # Features = what the rf will use\
    # Remove hyd, axis = 1 refers to columns, unsure what it does
    features = features.drop('hyd', axis=1)
    ...
    feature_list = np.delete(feature_list, np.where(feature_list == "hyd"))
    ...
    # Run optimisation
    rf_opt = rcv(estimator=rf, param_distributions=grid_param, n_iter=1000, cv=5, verbose=0, random_state=seed * n,
    ...
    # Store values to csv
    stored_preds.to_csv("stored_predictions_hyd.csv")
    stored_opt.to_csv("stored_opt_hyd.csv")
    stored_import.to_csv("stored_import_hyd.csv")
    stored_metrics.to_csv("stored_metrics_hyd.csv")
